 ...
 * the chatbotChatbotConfig should be defined in the html file before this script is included
 * the chatbotChatbotConfig should contain the token of the chatbot
...
async function embedChatbot () {
  const chatbotChatbotConfig = window.chatbotChatbotConfig;
  if (!chatbotChatbotConfig || !chatbotChatbotConfig.token) {
    console.error('chatbotChatbotConfig is empty or token is not provided')
    return;
  }
  const isDev = !!chatbotChatbotConfig.isDev
  const baseUrl = chatbotChatbotConfig.baseUrl || `https://${isDev ? 'dev.' : ''}uchatbot.app`
  const openIcon = `<svg
            id="openIcon"
            width="24"
            height="24"
  ...
  function createIframe () {
    ...
    iframe.title = "chatbot chatbot bubble window"
    iframe.id = 'chatbot-chatbot-bubble-window'
    iframe.src = `${baseUrl}/chatbot/${chatbotChatbotConfig.token}`
  ...
  const targetButton = document.getElementById('chatbot-chatbot-bubble-button')
  if (!targetButton) {
    // create button
    const containerDiv = document.createElement("div");
    containerDiv.id = 'chatbot-chatbot-bubble-button'
    ...
    containerDiv.addEventListener('click', function () {
      const targetIframe = document.getElementById('chatbot-chatbot-bubble-window')
      if (!targetIframe) {
        createIframe()
